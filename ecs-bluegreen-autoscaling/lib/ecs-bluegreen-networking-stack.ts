...
export class EcsBluegreenNetworkingStack extends cdk.Stack {
    ...
    public readonly productionListenerRuleArn: string;
    ...
    constructor(scope: Construct, id: string, props?: EcsBluegreenNetworkingStackProps) {
        ...
        // Restrict to the specified IP if provided, otherwise allow open access
        if (yourIpAddress !== '0.0.0.0/0') {
            ...
            this.albSecurityGroup.addIngressRule(
                ec2.Peer.anyIpv4(),
                ec2.Port.tcp(80),
                'Allow HTTP traffic from anywhere (no IP restriction specified)'
            );
        }

        NagSuppressions.addResourceSuppressions(this.albSecurityGroup, [{
            id: "AwsSolutions-EC23",
            reason: "ALB access is restricted by IP when provided, open access is used only for demo convenience",
        }]);
        ...
        // ALB Listener - production traffic only
        this.mainListener = this.alb.addListener('ALBListener', {
            port: 80,
            protocol: elbv2.ApplicationProtocol.HTTP,
            open: false, // We manage ingress rules ourselves above
            defaultAction: elbv2.ListenerAction.fixedResponse(503, {
                messageBody: 'Service unavailable',
            }),
        });

        // Explicit listener rule for routing (gives us a referenceable rule ARN for blue/green)
        ...
        this.productionListenerRuleArn = productionRule.listenerRuleArn;
